...
async def test_patched():
    ...
    for min_score in [0.0, 30.0, 40.0, 45.0, 50.0, 60.0]:
        alerted_txs = set()
        for a in alerts:
            if a.alert_type != "BEHAVIORAL_ANOMALY":
                # Add all transaction IDs from this alert
                alerted_txs.update(a.transaction_ids)
                
        tp = fp = fn = tn = 0
        for tx in labeled:
            label = tx.get("is_suspicious", False)
            tx_id = tx.get("id")
            score = tx.get("final_risk_score", 0.0)
            
            pred = (tx_id in alerted_txs)
            if min_score > 0:
                pred = pred or (score >= min_score)
        ...
        print(f"Typology Alert OR Score >= {min_score}: TP={tp}, FP={fp}, FN={fn}, TN={tn} | Prec={precision*100:.2f}%, Rec={recall*100:.2f}%, F1={f1*100:.2f}%, Acc={accuracy*100:.2f}%")
